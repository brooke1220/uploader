...
class Uploader extends Manager implements contract\Factory
{
    ...
    public function createLocalDriver()
    {
        ...
        return app(driver\Local::class, [$this->filesystems['disks']['local']]);
    }

    public function createUpyunDriver()
    {
        if (!isset($this->filesystems['disks'][$this->filesystems['default']])) {
            throw new \Exception('又拍云存储驱动不存在');
        }

        return app(driver\Upyun::class, [$this->filesystems['disks']['upyun']]);
    }
}
